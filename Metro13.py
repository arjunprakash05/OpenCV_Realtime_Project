...
while(f<=loopFrame) :
    ...
    lower = (5, 120, 210) #24,68,240
    upper = (35, 165, 245)
    #print hsv[400,340]
    #cv2.circle(hsv,(340,400),4,(255,255,255),-1)
    #cv2.imshow("hsv",hsv)
    mask = cv2.inRange(hsv, lower, upper)
    mask=mask.copy()[0:height,width/2-150:width/2]
...
#     
    ...
    offset_x_y0=float(float(0-y2)/float(slope)) + x2 + (width/2-150)
    offset_x_yheight=float(float(height-y2)/float(slope)) + x2 + (width/2-150)
    ...
    found=hog.detectMultiScale(roi_platform, winStride=(4,4), padding=(0,0), scale=1.05)[0]
    col=(0,255,0)
    for x, y, w, h in found:
        ...
        line_x_old=((bod_y+bod_h)-y2)/slope + x2 + (width/2-150)
        line_x=float(float(((bod_y+bod_h)-y2))/float(slope)) + x2 + (width/2-150)
    ...
    cv2.line(image,(x1+width/2-150,y1),(x2+width/2-150,y2),col,2)
