...
while(f<=loopFrame) :
    ...
    lower = (1, 130, 190) #24,68,240
    upper = (45, 190, 250)
    #print hsv[400,630]
    #cv2.circle(hsv,(630,400),4,(255,255,255),-1)
    #cv2.imshow("hsv",hsv)
    mask = cv2.inRange(hsv, lower, upper)
    mask=mask.copy()[0:height,width/2-290:width/2]
...
#     
    ...
    offset_x_y0=float(float(0-y2)/float(slope)) + x2 + (width/2-290)
    offset_x_yheight=float(float(height-y2)/float(slope)) + x2 + (width/2-290)
    ...
    for x, y, w, h in found:
        ...
        line_x_old=((bod_y+bod_h)-y2)/slope + x2 + (width/2-290)
        line_x=float(float(((bod_y+bod_h)-y2))/float(slope)) + x2 + (width/2-290)
    ...
    cv2.line(image,(x1+width/2-290,y1),(x2+width/2-290,y2),col,2)
