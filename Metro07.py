...
while(f<=loopFrame) :
    ...
    mask=mask.copy()[0:height,10:width/2-60]
...
#     
    ...
    offset_x_y0=float(float(0-y2)/float(slope)) + x2 + (10)
    offset_x_yheight=float(float(height-y2)/float(slope)) + x2 + (10)
    ...
    for x, y, w, h in found:
        ...
        line_x_old=((bod_y+bod_h)-y2)/slope + x2 + (10)
        line_x=float(float(((bod_y+bod_h)-y2))/float(slope)) + x2 + (10)
    ...
    cv2.line(image,(x1+10,y1),(x2+10,y2),col,2)
