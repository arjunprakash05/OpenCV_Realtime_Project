...
while(f<=loopFrame) :
    ...
    lower = (5, 110, 175) #24,68,240
    upper = (31, 150, 225)
    #print hsv[400,330]
    #cv2.circle(hsv,(330,400),4,(255,255,255),-1)
    #cv2.imshow("hsv",hsv)
    mask = cv2.inRange(hsv, lower, upper)
    mask=mask.copy()[0:height,width/2-230:width/2]
...
#     
    ...
    offset_x_y0=float(float(0-y2)/float(slope)) + x2 + (width/2-230)
    offset_x_yheight=float(float(height-y2)/float(slope)) + x2 + (width/2-230)
    ...
    for x, y, w, h in found:
        ...
        line_x_old=((bod_y+bod_h)-y2)/slope + x2 + (width/2-230)
        line_x=float(float(((bod_y+bod_h)-y2))/float(slope)) + x2 + (width/2-230)
    ...
    cv2.line(image,(x1+width/2-230,y1),(x2+width/2-230,y2),col,2)
