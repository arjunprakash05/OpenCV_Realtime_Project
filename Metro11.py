...
while(f<=loopFrame) :
    ...
    lower = (5, 150, 220) #24,68,240
    upper = (35, 190, 255)
    #print hsv[400,455]
    #cv2.circle(hsv,(455,400),4,(255,255,255),-1)
    #cv2.imshow("hsv",hsv)
    mask = cv2.inRange(hsv, lower, upper)
    mask=mask.copy()[0:height,width/2-170:width/2+100]
...
#     
    ...
    offset_x_y0=float(float(0-y2)/float(slope)) + x2 + (width/2-170)
    offset_x_yheight=float(float(height-y2)/float(slope)) + x2 + (width/2-170)
    ...
    for x, y, w, h in found:
        ...
        line_x_old=((bod_y+bod_h)-y2)/slope + x2 + (width/2-170)
        line_x=float(float(((bod_y+bod_h)-y2))/float(slope)) + x2 + (width/2-170)
    ...
    cv2.line(image,(x1+width/2-170,y1),(x2+width/2-170,y2),col,2)
