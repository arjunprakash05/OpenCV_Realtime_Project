...
while(f<=loopFrame) :
    ...
    lower = (14, 53, 225) #24,68,240
    upper = (34, 83, 255)
    mask = cv2.inRange(hsv, lower, upper)
    mask=mask.copy()[0:height,width/2-300:width/2-150]
...
#     
    ...
    offset_x_y0=float(float(0-y2)/float(slope)) + x2 + (width/2-300)
    offset_x_yheight=float(float(height-y2)/float(slope)) + x2 + (width/2-300)
    ...
    for x, y, w, h in found:
        ...
        line_x_old=((bod_y+bod_h)-y2)/slope + x2 + (width/2-300)
        line_x=float(float(((bod_y+bod_h)-y2))/float(slope)) + x2 + (width/2-300)
    ...
    cv2.line(image,(x1+width/2-300,y1),(x2+width/2-300,y2),(0,255,0),2)
